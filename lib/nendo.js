 ...
 * @param  {String}      fromFormat 開始年月のフォーマット 既定値 'YYYY/M'
 * @param  {String}      toFormat   終了年月のフォーマット 既定値 '-YYYY/M'
...
function formatNendo (date, fromFormat, toFormat, reverse) {
  ...
  fromFormat = unsetFrom ? '{YYYY}/{M}' : fromFormat;
  toFormat = unsetTo ? (unsetFrom ? '-{YYYY}/{M}' : null) : toFormat;
