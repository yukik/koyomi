...
function suji (num, type) {
  ...
  if (typeof num === 'string') {
    ...
    switch(type) {
    case '漢数字':
      return num.replace(DIGIT, x => kansuji(+x));
    case '漢字':
      return num.replace(DIGIT, x => kanji(+x));
    case '全角':
      return num.replace(DIGIT, x => zenkaku(+x));
    case '半角':
      return num.replace(ZNUM_REG, x => ZNUM.indexOf(x));
    case '序数':
      if (/^0+$/.test(num)) {
        return num + 'th';
      }
      return num.replace(DIGIT, x => x === '0' ? '0' : josu(+x));
    default:
      return reverse(num);
    }
 ...
 */
function kanji (num) {
  return ('' + num).split('').map(x => JNUM[x * 1]).join('');
}
...
function zenkaku (num) {
  return ('' + num).split('').map(x => ZNUM[x * 1]).join('');
}
