...
var formatLimit = Koyomi.format.bind(Koyomi);
testAll(false, formatLimit);

var format = koyomi.format.bind(koyomi);
testAll(true, format);


function testAll(unlimit, format) {
  ...
  if (unlimit) {
    ...
    // closed
    test(format('2015-1-1' , 'closed'), '祝日');
    test(format('2015-1-2' , 'closed'), '休業日');
    test(format('2015-1-4' , 'closed'), '定休日');
    test(format('2015-1-5' , 'closed'), '');
  }
