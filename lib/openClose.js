...
function isOpened (date) {
  ...
  // 強制設定
  var opened = this.checkDayInfoHoliday(date);
  if (opened !== null) {
    return opened;
  }
 ...
 *                             休業日設定   false
 */
function checkDayInfoHoliday (date) {
  var info = this.getDayInfo(date);
...
module.exports = function (Koyomi) {
  ...
  Koyomi.prototype.checkDayInfoHoliday = checkDayInfoHoliday;
