 ...
 *                   1月は1です
 *   4. 数字         YYYYMMDDHHmmss の形式とみなす
 *   5. オブジェクト (year, month, dayのプロパティを持つ)
 *   
 * 文字列からDateに対応
 * 和暦から西暦にも対応
 * @method toDate
 * @param  {Date|String|Array|Object} value
...
function toDate(value, trim) {
  ...
  // 省略時は現在時刻
  if (!value) {
    return trim ? trimDate(new Date()): new Date();
  ...
  // 配列
    ...
    value = new Date(v[0], v[1] - 1, v[2], v[3], v[4], v[5]);
