...
module.exports = function (Koyomi) {
  ...
  Koyomi.prototype.getAge         = getAge;
};
...
var getTani = require('./fx/getTani');
...
function yearDays (date) {
  ...
  var startMonth = CONFIG.START_MONTH;
  var year = date.getFullYear();
  if (date.getMonth() + 1 < startMonth) {
    return diffDays([year - 1, startMonth, 1], [year, startMonth, 1]);
  } else {
    return diffDays([year, startMonth, 1], [year + 1, startMonth, 1]);
  }
}
...
function passYearDays (date) {
  ...
  var year = date.getFullYear();
  if (date.getMonth() + 1 < startMonth) {
    return diffDays([year - 1, startMonth, 1], date) + 1;
  } else {
    return diffDays([year, startMonth, 1], date) + 1;
  }
}
